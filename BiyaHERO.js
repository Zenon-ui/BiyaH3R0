...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  class hazardtype{
      static Pothole = new hazardtype("Pothole","🕳️",true,true);
      ...
      static Lightning = new hazardtype("Lightning","⚡",true,true);
          constructor(label,emoji,weighted,canblock){
          this.label = label;
          this.emoji = emoji;
...
(function(){
  ...
  async function attemptSignup(){
    ...
    // Offline signup: queue it instead of blocking. There's still no way
    // to hand out a real, server-verified Supabase account with zero
    // network — but there's no reason a commuter opening this app for
    // the first time in a dead zone should be locked out while everything
    // else (map, GPS, hazard reporting) already works offline. The
    // validated details are encrypted and stored locally (see
    // PendingAuth), the app lets them straight into the download/app
    // stage on a temporary local id, and the real signUp() call fires
    // automatically the moment this device sees a connection again —
    // see trySyncPendingAccount(), wired into goOnline() below.
    if (!navigator.onLine) {
      ...
      enterDownloadStage(name, email, { id: localId, pending: true });
  ...
  function updatePendingAccountBanner(){
    ...
    if (record.conflict) {
      ...
      subEl.textContent = `${record.email} is already registered. Log out and log in with that account's password, or contact an administrator.`;
    } else {
...
function resolveHazardGraphEdge(hazard){
    if (!hazard || hazard.graphEdgeid != null) return;
    const db = window.RoutingDatabase && window.RoutingDatabase.getDb && window.RoutingDatabase.getDb();
    if (!db || !window.AStarRouter) return;
    window.AStarRouter.matchHazardToEdge(db, hazard.lat, hazard.lng)
        .then(edgeId => {
            if (edgeId != null) {
                hazard.graphEdgeid = edgeId;
                saveDynamicHazards();
            }
        })
        .catch(() => {});
...
,
  ...
  function buildHazardEdgeWeightFn(){
    const weightByEdgeId = new Map();
    dynamicHazards.forEach(h => {
      if (h.graphEdgeid == null) return;
      // dynamicHazards holds plain objects loaded back from localStorage
      // (JSON round-trips lose the class), so rebuild a real `hazards`
      // instance to get its routingweightmult() logic rather than
      // duplicating the avoid/weight rules here.
      let mult = 1;
      try { mult = new hazards(h, h, h.locationNote).routingweightmult(); } catch (e) { mult = 1; }
      if (mult > 1) {
        weightByEdgeId.set(h.graphEdgeid, Math.max(weightByEdgeId.get(h.graphEdgeid) || 1, mult));
      }
    });
    if (!weightByEdgeId.size) return undefined;
    return (edge) => weightByEdgeId.get(edge.id) || 1;
  }

  // Offline, on-device routing tier: A* over the bundled road graph (see
  ...
  // to buildFallbackRoute()'s curved approximation exactly as it always
  // has. This doesn't touch or replace the OSRM path above — it only ever
  // runs after OSRM has already been ruled out (no connection, or the
  // request failed/timed out).
  async function tryLocalGraphRoute(origin, dest, modeCfg){
    const db = window.RoutingDatabase && window.RoutingDatabase.getDb && window.RoutingDatabase.getDb();
    if (!db || !window.AStarRouter) return null;
    try {
      const hazardWeightForEdge = buildHazardEdgeWeightFn();
      const result = await window.AStarRouter.findRoute(db, origin, dest, { hazardWeightForEdge });
      ...
      return { pts: result.pts, isReal: true, steps: [], stepStarts: [], baseDurationSec };
    } catch (err) {
  ...
  async function fetchRoadRoute(origin, dest, mode){
    const modeCfg = getTransportMode(mode);
    if (!navigator.onLine) {
      const graphRoute = await tryLocalGraphRoute(origin, dest, modeCfg);
      return graphRoute || buildFallbackRoute(origin, dest, modeCfg);
    }
